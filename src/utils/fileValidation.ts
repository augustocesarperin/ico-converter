...
const ERROR_MESSAGES = {
  ...
  DIMENSIONS_TOO_SMALL: (minWidth: number, minHeight: number, currentWidth: number, currentHeight: number) => 
    `Imagem muito pequena (${currentWidth}×${currentHeight}px). Mínimo: ${minWidth}×${minHeight}px`,
  DIMENSIONS_TOO_LARGE: (maxWidth: number, maxHeight: number, currentWidth: number, currentHeight: number) => 
  ...
  RATE_LIMIT: 'Muitos uploads em pouco tempo. Aguarde um momento antes de tentar novamente.',
  PROCESSING_TIMEOUT: 'Processamento demorou muito tempo. Tente com uma imagem menor.'
};
...
const getImageDimensionsSecure = (file: File, timeout: number = 10000): Promise<{ width: number; height: number }> => {
...
export const validateImageFile = async (
  ...
  try {
    if (!checkRateLimit()) {
      return {
        isValid: false,
        error: t ? t('hero.errors.validation.rate_limit') : ERROR_MESSAGES.RATE_LIMIT,
        warnings: []
      };
    }
  ...
  if (!opts.allowedTypes.includes(file.type)) {
      const errorMsg = t 
        ? t('hero.errors.validation.file_type', { allowedTypes: opts.allowedTypes.map(type => type.split('/')[1].toUpperCase()).join(', ') })
  ...
  if (fileSizeInMB > opts.maxSizeInMB) {
      const errorMsg = t
        ? t('hero.errors.validation.file_size', { currentSize: fileSizeInMB.toFixed(1), maxSize: opts.maxSizeInMB })
    ...
    if (dimensions && (dimensions.width < opts.minDimensions.width || dimensions.height < opts.minDimensions.height)) {
      const errorMsg = t
        ...
        : ERROR_MESSAGES.DIMENSIONS_TOO_SMALL(opts.minDimensions.width, opts.minDimensions.height, dimensions.width, dimensions.height);
      return {
        isValid: false,
        error: errorMsg,
        warnings,
    ...
    if (dimensions && (dimensions.width > opts.maxDimensions.width || dimensions.height > opts.maxDimensions.height)) {
       const errorMsg = t
        ...
        : ERROR_MESSAGES.DIMENSIONS_TOO_LARGE(opts.maxDimensions.width, opts.maxDimensions.height, dimensions.width, dimensions.height);
      return {
        isValid: false,
        error: errorMsg,
        warnings,
...
export const getErrorSuggestion = (errorType: string, t?: TFunction): string => {
