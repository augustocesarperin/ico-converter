...
export const SECURITY_CONFIG = {
  ...
  VALIDATION: {
    ...
    cssColorRegex: /^(#[0-9a-fA-F]{3,8}|rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)|rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[\d.]+\s*\)|hsl\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*\)|hsla\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*,\s*[\d.]+\s*\)|[a-zA-Z]+)$/,
  ...
  ERROR_MESSAGES: {
    RATE_LIMIT: '⏱️ Calma aí! Muitos uploads seguidos. Aguarde alguns segundos para tentar novamente.',
    FILE_TOO_LARGE: '📏 Sua imagem é um pouco grande demais. Que tal reduzir o tamanho para menos de {maxSize}MB?',
    PROCESSING_TIMEOUT: '⏰ Ops! Sua imagem está demorando para processar. Tente com uma imagem menor ou em formato diferente.',
    MEMORY_LIMIT: '🧠 Imagem muito complexa para processar. Tente redimensionar para menos de 2048×2048 pixels.',
    CONCURRENT_PROCESSING: '⚡ Já estou processando uma imagem. Aguarde terminar para enviar outra!',
    INVALID_DIMENSIONS: '📐 Hmm, não consegui ler as dimensões desta imagem. Ela pode estar corrompida.',
    CORRUPTED_FILE: '🔧 Este arquivo parece estar com problemas. Tente salvar a imagem novamente ou use outro formato.',
    SECURITY_VIOLATION: '🛡️ Por segurança, não posso processar este arquivo. Tente com uma imagem PNG ou JPG normal.',
    FILE_TYPE_FRIENDLY: '📁 Este formato não é suportado. Use PNG, JPG ou WebP para melhores resultados!',
    SIZE_SUGGESTION: '💡 Dica: Para ícones, imagens entre 256×256 e 1024×1024 pixels funcionam perfeitamente!',
    PERFORMANCE_WARNING: '🚀 Imagem grande detectada! O processamento pode demorar um pouquinho mais.',
    QUALITY_TIP: '✨ Para melhor qualidade de ícone, use imagens quadradas (mesma largura e altura).'
  },
...
export const SecurityUtils = {
  ...
  isValidFilename: (filename: string): boolean => {
    ...
    if (filename.length > SECURITY_CONFIG.VALIDATION.maxFilenameLength) return false;
  ...
  isOperationSafe: (estimatedMemoryUsage: number, estimatedProcessingTime: number): boolean => {
...
export const UXHelpers = {
  ...
  getFriendlyErrorMessage: (errorType: string, context?: any): string => {
    const messages = SECURITY_CONFIG.ERROR_MESSAGES;
    
    switch (errorType) {
      case 'FILE_TOO_LARGE':
        return messages.FILE_TOO_LARGE.replace('{maxSize}', context?.maxSize || '50');
      case 'FILE_TYPE':
      ...
      default:
        return messages[errorType as keyof typeof messages] || messages.SECURITY_VIOLATION;
    }
  },
  ...
  getProgressiveHelp: (attemptCount: number, lastError?: string): string[] => {
    ...
    if (attemptCount === 1) {
      tips.push("💡 Primeira vez? Use imagens PNG ou JPG para melhores resultados!");
    }
    
    if (attemptCount === 2 && lastError?.includes('grande')) {
      tips.push("🔧 Tente redimensionar sua imagem para 1024×1024 pixels ou menos.");
      tips.push("📱 Você pode usar qualquer editor de imagem ou até mesmo o Paint!");
    }
    
    if (attemptCount >= 3) {
      tips.push("🆘 Precisa de ajuda? Tente com uma imagem simples primeiro (ex: PNG de 512×512px).");
  ...
  checkRateLimitWithWarning: (): { allowed: boolean; warning?: string; remaining?: number } => {
  ...
  getSmartSuggestions: (file: File, error?: string): string[] => {
    ...
    if (sizeInMB > 5) {
      suggestions.push("💾 Comprima sua imagem online (TinyPNG, Squoosh.app) para reduzir o tamanho.");
    }
    
    if (file.type === 'image/bmp' || file.type === 'image/tiff') {
      suggestions.push("🔄 Converta para PNG ou JPG - formatos mais eficientes e compatíveis.");
    }
    
    if (error?.includes('timeout')) {
      suggestions.push("⚡ Tente uma imagem menor ou em formato PNG para processamento mais rápido.");
    }
