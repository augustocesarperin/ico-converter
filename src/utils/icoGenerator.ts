...
const resizeWithSteps = (img: HTMLImageElement, destWidth: number, destHeight: number): ImageData => {
    ...
    finalCtx.drawImage(currentCanvas, 0, 0, currentWidth, currentHeight, 0, 0, destWidth, destHeight);

    return finalCtx.getImageData(0, 0, destWidth, destHeight);
};

const imageDataToBlob = async (imageData: ImageData, quality: number): Promise<Blob> => {
...
const processImageSecurely = async (
  ...
  try {
    ...
    if (totalPixels > SECURITY_LIMITS.maxCanvasSize / 4) {
      throw new Error('Imagem muito grande. Use uma imagem menor para evitar problemas de performance.');
    }
...
const generateIcoWithMemoryTracking = async (
  ...
  for (let i = 0; i < sizes.length; i++) {
    ...
    if (currentMemory > SECURITY_LIMITS.maxMemoryUsage) {
      throw new Error('Limite de memória excedido. Tente com uma imagem menor ou menos tamanhos.');
    }
    ...
    onProgress?.(15 + (i * progressStep), `Redimensionando para ${size}×${size}px...`);
  ...
  const pngs = await Promise.all(resizedImages.map(async ({ size, imageData }) => {
...
export const downloadIcoFile = (icoBlob: Blob, filename: string = 'icon.ico') => {
